...
int make_WS3_Network (int** Network, int N, int int_k, int *color) {
	...
	for(i=0;i<3;i++){
		Act[i] = (int*)malloc(sizeof(int)*(N/3));
		An[i] = 0;
	}

	color = (int*)realloc(color, sizeof(int)*N);
	l = n = m = 0;
	for(i=0;i<N;i++){
		color[i] = i % 3;
		Act[color[i]][An[color[i]]++] = i;
	}		

	Network = (int**)realloc(Network, sizeof(int*)*N);
	for(i=0;i<N;i++){
		Network[i] = (int*)malloc(sizeof(int)*3);
		Network[i][0] = 2;
		Network[i][1] = (i+1)%N;
		Network[i][2] = (i-1+N)%N;
	}
	...
	for(i=0;i<N;i++){
		...
		if ( An[l] == 1 && Act[l][0] == k ) {
			// rewiring!
			while ( check ) {
				j = (int)(genrand64_real2()*(N/3))*3 + (i%3);
				if ( j == i ) continue;
				check = 0;
				for( n=1;n<=Network[j][0];n++){
					if ( k == Network[j][n] ) {
						check = 1;
						break;
					}
			...
			for(n=3;n<=Network[j][0];n++){
				if ( color[Network[j][n]] == l ) {
					m = Network[j][n];
					Network[j][n] = k;
					Network[k] = (int*)realloc(Network[k], sizeof(int)*(++Network[k][0]+1));
					Network[k][Network[k][0]] = j;
					break;
				}
			}
			for(n=3;n<=Network[m][0];n++){
				if ( Network[m][n] == j ) {
					Network[m][n] = i;
					Network[i] = (int*)realloc(Network[i], sizeof(int)*(++Network[i][0]+1));
					Network[i][Network[i][0]] = m;
					break;
				}
			}
		}
			...
			while ( m == k ) {
				j = (int)(genrand64_real2()*(double)An[l]);
				m = Act[l][j];
			}
			Act[l][j] = Act[l][--An[l]];
			Network[i] = (int*)realloc(Network[i], sizeof(int)*(++Network[i][0]+1));
			Network[m] = (int*)realloc(Network[m], sizeof(int)*(++Network[m][0]+1));
			Network[i][Network[i][0]] = m;
			Network[m][Network[m][0]] = i;
		}
	}
	for(i=0;i<3;i++){
		free(Act[i]);
...
int make_Triangular( int** Network, int L ) {
	...
	for(k=0;k<N;k++){
		Network[k][1] = (k / L)*L + (k+1) % L;
		Network[k][2] = (k / L)*L + (k-1+L) % L;
		Network[k][3] = (k+L) % N;
		Network[k][4] = (k-L+N) % N;
		Network[k][5] = ((k+L)%N /L)*L + (k+1)%L;
		Network[k][6] = ((k-L+N)%N /L)*L + (k-1+L)%L;
	}
	return N;
}
...
int Conf_SF ( int** Network, int N, double mean_k, double gamma ) {
	...
	for(i=0;i<N;i++){
		tmp_network[i] = (int*)malloc(sizeof(int)*1);
		tmp_network[i][0] = 0;
		Degrees[i] = (int)(power_law(1., 2.5));
		totdeg += Degrees[i];
	}
	
	Actives = 0;
	Active = (int*)malloc(sizeof(int)*(totdeg));
	for(i=0;i<N;i++){
	...
	while ( Actives > 20 ) {
		...
		while ( lg_n == sm_n || check ) {
			check = 0;
			sl1 = (int)(genrand64_real2()*(double)Actives);
			sl2 = (int)(genrand64_real2()*(double)Actives);
			lg_n = Active[sl1];
			sm_n = Active[sl2];
			if ( lg_n == sm_n ) continue;
			for(i=1;i<=tmp_network[lg_n][0];i++){
				if ( tmp_network[lg_n][i] == sm_n ) {
					check = 1;
					break;
				}
			}
		...
		Active[sl1] = Active[--Actives];
		Active[sl2] = Active[--Actives];
